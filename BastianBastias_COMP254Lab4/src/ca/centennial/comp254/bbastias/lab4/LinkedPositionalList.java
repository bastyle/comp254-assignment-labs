...
public class LinkedPositionalList<E> implements PositionalList2<E> {
	...
	@Override
	public long indexOf(Position<E> p) throws IllegalArgumentException {
		Position<E> current = first();// setting the variable "current" to the first position of the linked list by
										// calling the "first" method.
		int index = 0;
		while (current != null) {// iterate through each position in the linked list until it either finds the
									// desired position (p) or reaches the end of the list (i.e., current == null).
			if (current == p) {// checks if the current position is equal to the desired position
				return index;
			}
			index++;// If the current position is not equal to the desired position, the code
					// increments the index by 1
			current = after(current);// moves to the next position in the list by calling the "after" method with the
										// current position as a parameter.
		}
