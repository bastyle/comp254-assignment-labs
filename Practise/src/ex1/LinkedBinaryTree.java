...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	int pathLengthUtil(Node_ node1, Auxi res) {

		if (node1 == null)
			return 0;

		int l = pathLengthUtil(node1.leftValue, res);
		int r = pathLengthUtil(node1.rightValue, res);

		int maxS = Math.max(Math.max(l, r) + node1.data, node1.data);

		int maxT = Math.max(maxS, l + r + node1.data);

		res.value = Math.max(res.value, maxT);

		return maxS;
	}

//Method that computes the path length of a tree T
	int pathLength(Node_ node1) {

		Auxi res = new Auxi();
		res.value = Integer.MIN_VALUE;

		pathLengthUtil(node1, res);
		return res.value;
	}

	public static void main(String[] args) {
		BinaryTree_ bTree = new BinaryTree_();
