...
public class SortedTableMap<K, V> extends AbstractSortedMap<K, V> {
	...
	/**
	 * Returns the smallest index for range table[low..high] inclusive storing an
	 * entry with a key greater than or equal to the given k. If no such element
	 * exists, returns index high+1 by convention.
	 * 
	 ...
	 *         key (or index high+1 if no such entry exists)
	 */
	...
	/*
	 * public boolean containKey(K key) { // use findIndex method to check for the
	 * key in your map int index = findIndex(key);
	 * System.out.println("index: "+index); if (findIndex(key) == -1) return false;
	 * // return true; }
	 */
	...
	public boolean containKey(K key) {
		...
		// Otherwise, the key is in the map.
