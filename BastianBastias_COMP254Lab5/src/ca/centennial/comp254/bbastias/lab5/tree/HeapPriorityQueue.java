...
//	}
	...
	protected void upheap(int j) {
		if (j > 0) { // (base case)checks if we haven't reached the root node yet. If j is already 0,
						// we don't need to do anything else, so we can exit the recursion.
			int p = parent(j);// finds the index of j's parent node.
			if (compare(heap.get(j), heap.get(p)) < 0) {// checks if the heap property is violated between j and its
														// parent p. If j is less than p, we need to swap their
														// positions to restore the heap property.
				swap(j, p);// exchanges the values stored at indices j and p in the heap list.
				upheap(p); // makes a recursive call to upheap() on the parent's index p. 
			}
		}
	}
