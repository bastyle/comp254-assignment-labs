...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	 ...
	 */
	private Position<Entry<K, V>> treeSearchOri(Position<Entry<K, V>> p, K key) {
		System.out.println("treeSearch key: " + key);
		if (isExternal(p))
		...
		if (comp == 0)
			...
			return treeSearchOri(left(p), key); // search left subtree
		else
			return treeSearchOri(right(p), key); // search right subtree
	}

	protected Position<Entry<K, V>> treeSearch(Position<Entry<K, V>> p, K key) {
		System.out.println("searching key: " + key);
		while (isInternal(p)) { // while p is not an external node
			int compResult = compare(key, p.getElement());
			if (compResult == 0) { // key found; return its position
				return p;
			} else if (compResult < 0) { // checks whether compare result is negative, meaning that the key is smaller
											// than the element at p
				p = left(p); // going to look into the left subtree.
			} else {
				p = right(p); // going to look into the right subtree.
			}
		}
		return p;
	}
